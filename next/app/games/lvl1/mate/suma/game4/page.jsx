...
const SumGamePage4 = () => {
  ...
  const updateQuestionCount = (newQuestionCount) => {
    ...
    // Si llegamos al final del juego, mostrar el mensaje final
    if (newQuestionCount === 10) {
      const passingScore = 120; // Límite de estrellas para pasar
      let message = `Fin del juego. Has obtenido ${score} estrellas. `;
  ...
  const retryGame = () => {
    ...
    setQuestionCount(1);
    setFinalMessage('');
    // Vuelve a iniciar el juego después de un pequeño retraso para asegurar la limpieza del estado anterior
      ...
      {gameStarted && (
        <section className='min-h-screen flex flex-col items-center'>
          <div className="my-16 p-6 story bg-white rounded-lg shadow-lg w-[850px]">
            <h1 className="text-3xl font-bold mb-6 text-center">Sumas Avanzadas</h1>
            <div className="flex justify-between text-2xl mt-8">
              <p className="text-2xl">Pregunta: {questionCount}/10</p>
              <p className="text-2xl">Estrellas: {score}/200</p>
            </div>
            ...
            {finalMessage && (
              <div className="mt-6 p-4 bg-green-100 rounded-lg">
                <h2 className="text-2xl font-semibold">Resultado Final:</h2>
                <p className='text-xl'>{finalMessage}</p>
                {/* Mostrar botón de reintentar solo si el jugador no pasó la actividad */}
                {score < 120 && (
                  <button 
                    onClick={retryGame} 
