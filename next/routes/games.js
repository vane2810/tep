...
router.post('/', async (req, res) => {
    const { title, img_url, contentId } = req.body; // Asegúrate de que el `contentId` esté bien escrito.
    try {
        const newGame = await Game.create({ title, img_url, contentId });
...
router.put('/:id', async (req, res) => {
    const { title, img_url, contentId } = req.body;
    try {
        const game = await Game.findByPk(req.params.id);
        if (game) {
            await game.update({ title, img_url, contentId });
            res.status(200).json(game);
        } else {
            res.status(404).json({ message: 'Juego no encontrado' });
        }
