...
const GameCard = ({
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        // Si el rol no es estudiante, desbloquear siempre el juego
    ...
    fetchProgress();
  }, [studentId, previousGameId, userRole]);
  ...
  return (
    <div
      className={`relative flex flex-col justify-between items-center border-2 border-gray-100 shadow-xl hover:shadow-2xl p-8 rounded-xl w-full max-w-md text-center transform transition-all hover:scale-105 ${
        isUnlocked ? 'bg-white' : 'bg-gray-300 opacity-75'
      }`}
    >
...
GameCard.propTypes = {
  ...
  previousGameId: PropTypes.number,
  userRole: PropTypes.string.isRequired, // Añadido para recibir el rol del usuario
};
