...
const MapIngles = ({ subject, basePath, levels, fondoUrl, decorativos, caricatura }) => (
            ...
            <div key={level.id} className="absolute" style={{ top: level.position.top, left: level.position.left }}>
                <Link href={`/${basePath}${subject}/${level.id}`}>
                    <img
                        src={level.img}
                        alt={`Nivel ${level.id}`}
                        className="hover:shadow-lg rounded-full w-[6vw] h-[6vw] transform transition-transform cursor-pointer hover:scale-105"
                        title={`${level.name}`}
                    />
                </Link>
