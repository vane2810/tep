...
export default function ModalEliminarUsuario({ onClose, usuario }) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setTimeout(() => {
          ...
          onClose(true); // Indicamos que la eliminación fue exitosa
  ...
  return (
    <>
      {isModalVisible && (
        <div className="z-40 fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 transition-opacity duration-300">
          <div className="relative bg-white shadow-2xl p-8 rounded-lg w-full max-w-md text-center">
            {isSuccess ? (
              <>
                <FiTrash2 className="mx-auto mb-4 text-green-500" size={50} />
                <p className="mb-4 font-semibold text-center text-green-500">{mensaje}</p>
                <button
                  onClick={() => {
                    setIsModalVisible(false);
                    onClose(true); // Al cerrar el modal indicamos que fue un éxito
                  }}
                  className="bg-blue-500 hover:bg-blue-600 px-4 py-2 rounded-full font-bold text-white transition-all duration-200"
                >
                  Aceptar
                </button>
              ...
              <>
                ...
                <p className="mb-6 text-gray-600">
                  ...
                  <span className="font-bold">{usuario?.name}</span>? <br />
                  Esta acción no se puede deshacer.
                </p>
                {mensaje && (
                  <div
                    className={`p-3 mb-4 rounded text-center font-semibold text-white transition-all duration-300 ${mensaje.includes("exitosamente") ? "bg-green-500" : "bg-red-500"
                      }`}
                  >
                    {mensaje}
                  </div>
