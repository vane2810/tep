...
const Game1 = ({ updateFeedback, updateScore, proceedToNextScene, isFinalScene, finalScore, restartGame }) => {
    ...
    const generateQuestion = function() {
        ...
        // Mostrar la pregunta
        this.add.text(this.sys.game.config.width / 2, 100, `¿Cuál es la fracción que tiene el numerador ${numerator} y denominador ${denominator}?`, {
            fontSize: '20px',
            fill: '#ffffff',
            align: 'center',
    ...
    const checkAnswer = function(selectedOption, button) {
        let feedbackMessage = '';
        let feedbackColor = '';
        let score = 0;
        ...
        if (!isFinalScene) {
            const nextButton = this.add.text(this.sys.game.config.width / 2, 350, 'Siguiente', { // Ajustado a 350px desde la parte superior
                fontSize: '20px',
                fill: '#ffffff',
                backgroundColor: '#7966ab',
                padding: { x: 20, y: 10 }
            ...
            nextButton.on('pointerdown', () => {
                ...
                setGameInstance(null); // Permitir que se cree una nueva instancia
            });
        } else {
            ...
            const finalMessage = this.add.text(this.sys.game.config.width / 2, 350, finalMessageText, { // Ajustado a 350px desde la parte superior
                fontSize: '20px',
                fill: '#ffffff',
            ...
            if (finalScore < 60) {
                ...
                const retryButton = this.add.text(this.sys.game.config.width / 2, 400, 'Volver a Intentarlo', { // Ajustado a 400px desde la parte superior
                    fontSize: '20px',
                    fill: '#ffffff',
