...
const EncontrarParejaForm = ({ isOpen, onClose, gameData, onSave, isEditing, existingConfig }) => {
  ...
  const validateForm = () => {
    ...
    if (pairs.length === 0) newErrors.push("Debe agregar al menos un par de textos.");
    if (description.trim() === "") newErrors.push("Debe proporcionar una descripción del juego.");

    pairs.forEach((pair, index) => {
      if (!pair.text1 || !pair.text2) newErrors.push(`El par #${index + 1} tiene textos faltantes.`);
    });
  ...
  const isGeneralConfigComplete = () => {
    return points > 0 && timeLimit > 0 && description.trim() !== "";
  };
  ...
  return (
    <div className="z-50 fixed inset-0 flex justify-center items-center bg-black bg-opacity-50 yagora">
      <div className="bg-white shadow-lg p-8 rounded-lg w-full max-w-3xl max-h-screen text-black overflow-auto">
        <h2 className="mb-4 font-bold text-2xl text-center">{isEditing ? "Editar Juego Encontrar la Pareja" : "Configurar Juego Encontrar la Pareja"}</h2>
          ...
          <div>
            <div className="relative mb-4">
              <label className="block mb-1 font-bold">
                ...
                <FaInfoCircle
                  ...
                  onClick={(e) => handleTooltip(e, "Puntos que el jugador puede obtener si completa el juego correctamente")}
            ...
            <div className="relative mb-4">
              <label className="block mb-1 font-bold">
                ...
                <FaInfoCircle
                  className="inline ml-2 text-purple-500 cursor-pointer"
                  onClick={(e) => handleTooltip(e, "Breve descripción sobre el objetivo del juego")}
                  onMouseLeave={hideTooltip}
                />
              </label>
              <textarea
          ...
          <div>
            ...
            {pairs.map((pair, index) => (
              <div key={index} className="border-gray-300 shadow-sm mb-6 p-4 border rounded-lg">
                ...
                <label className="block mb-1 font-bold">Par 2:</label>
                <input
                  type="text"
                  value={pair.text2}
...
export default EncontrarParejaForm;
