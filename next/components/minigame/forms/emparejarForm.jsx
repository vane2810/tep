...
const EmparejarForm = ({ isOpen, onClose, gameData, onSave, isEditing, existingConfig }) => {
  ...
  const [pointsPerPair, setPointsPerPair] = useState(0);
  ...
  useEffect(() => {
    if (isEditing && existingConfig) {
      try {
        ...
        setPointsPerPair(config.points_per_pair || 0);
  ...
  const validateForm = () => {
    ...
    if (pointsPerPair <= 0) newErrors.push("Puntos por par correcto deben ser mayores que 0.");
  ...
  const isGeneralConfigComplete = () => {
    return points > 0 && pointsPerPair > 0 && pointsMin > 0;
  };
          ...
          <div>
            ...
            <div className="relative mb-4">
              <label className="block mb-1 font-bold">
                ...
                <FaInfoCircle
                  className="inline ml-2 text-purple-500 cursor-pointer"
                  onClick={(e) => handleTooltip(e, "La cantidad de puntos que se otorgan por cada par correcto")}
                  onMouseLeave={hideTooltip}
                />
              </label>
              <input
                type="number"
                value={pointsPerPair === 0 ? "" : pointsPerPair}
                onChange={(e) => setPointsPerPair(Number(e.target.value) || 0)}
                className="border-gray-300 p-2 border rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-purple-500"
              />
            </div>
