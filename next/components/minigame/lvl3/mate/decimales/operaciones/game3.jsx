// Juego 3 - OPeraciones básicas de fracciones - Nivel 3
"use client";
...
const Game3 = ({ updateFeedback, updateScore, finalizeGame, resetGame }) => {
    ...
    useEffect(() => {
        ...
        function createScene() {
            ...
            // Generar y mostrar las 5 filas de preguntas de operaciones básicas entre fracciones
            for (let i = 0; i < 5; i++) {
        ...
        function generateQuestion(rowIndex) {
            ...
            switch (operation) {
                case '+':
                    correctAnswer = `${numerator1 * denominator2 + numerator2 * denominator1}/${denominator1 * denominator2}`;
                    break;
                case '-':
                    correctAnswer = `${numerator1 * denominator2 - numerator2 * denominator1}/${denominator1 * denominator2}`;
                    break;
                case '*':
                    correctAnswer = `${numerator1 * numerator2}/${denominator1 * denominator2}`;
                    break;
                case '/':
                    correctAnswer = `${numerator1 * denominator2}/${numerator2 * denominator1}`;
                    break;
                default:
                    break;
            }
            ...
            this.add.text(400, 50 + rowIndex * 100, `Fila ${rowIndex + 1}: Resuelve ${numerator1}/${denominator1} ${operation} ${numerator2}/${denominator2}`, {
                fontSize: '18px',
                fill: '#ffffff',
                align: 'center',
                backgroundColor: '#7966ab',
        ...
        function checkAnswer(selectedAnswer, rowIndex, button) {
            ...
            if (selectedAnswer === correctAnswers[rowIndex]) {
                ...
                feedbackMessage = `¡Correcto! El resultado es ${selectedAnswer}.`;
