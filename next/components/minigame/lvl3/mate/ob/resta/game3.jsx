  ...
  useEffect(() => {
    class NumberHuntScene extends Phaser.Scene {
      ...
      create() {
        ...
        this.targetDifference = Phaser.Math.Between(1, 9); // Número objetivo aleatorio para la diferencia
        this.canSelect = true;

        this.generateNewQuestion();
        
        // Mostrar la diferencia objetivo al jugador
        this.targetDifferenceText = this.add.text(400, 50, `Encuentra dos números cuya diferencia sea ${this.targetDifference}`, {
          fontSize: '24px',
          fill: '#000000'
        }).setOrigin(0.5);
      ...
      selectNumber(numberText) {
        ...
        if (this.selectedNumbers.length === 2) {
          ...
          const difference = Math.abs(num1 - num2); // Calcular la diferencia absoluta

          if (difference === this.targetDifference) {
            memoizedUpdateFeedback('¡Correcto! Has encontrado una combinación que cumple con la diferencia objetivo.');
            ...
            memoizedUpdateFeedback('La diferencia no es correcta. Inténtalo de nuevo.');
            this.resetSelection();
          }
        }
      }
      ...
      moveToNextQuestion() {
        this.cleanUpScene();
        this.generateNewQuestion();
        this.targetDifference = Phaser.Math.Between(1, 9); // Nuevo número objetivo para la diferencia
        this.targetDifferenceText.setText(`Encuentra dos números cuya diferencia sea ${this.targetDifference}`);
        this.nextQuestionButton.setVisible(false); 
      }
