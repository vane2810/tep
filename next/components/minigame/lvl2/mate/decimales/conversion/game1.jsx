...
const Game1 = ({ updateFeedback, updateScore, proceedToNextScene, isFinalScene, finalScore, restartGame }) => {
    ...
    useEffect(() => {
        ...
        function generateQuestion() {
            const fraction = Phaser.Math.Between(1, 9);
            const decimal = (fraction / 10).toFixed(1); // Convierte a decimal con un solo decimal
            correctAnswer = decimal; // La respuesta correcta es el decimal correspondiente

            const wrongAnswers = [
                ((fraction + 1) / 10).toFixed(1),
                ((fraction - 1) / 10).toFixed(1),
                ((fraction + 2) / 10).toFixed(1)
            ];

            questions = [correctAnswer, ...wrongAnswers].sort(() => Math.random() - 0.5); // Mezclar respuestas

            // Mostrar la instrucción
            this.add.text(400, 50, `Convierte la fracción 1/${fraction * 10} a decimal`, {
                fontSize: '22px',
                fill: '#ffffff',
                align: 'center',
            ...
            questions.forEach((num, index) => {
                const button = this.add.text(220 + (index * 120), 150, num, {
                    fontSize: '28px',
                    fill: '#000000',
        ...
        function checkAnswer(selectedNum, button) {
            ...
            if (selectedNum == correctAnswer) {
                ...
                feedbackMessage = '¡Correcto! Has seleccionado el decimal correcto.';
