...
const Game2 = ({ updateFeedback, updateScore, isFinalScene, finalScore }) => {
    ...
    useEffect(() => {
        ...
        const targetQuotient = 0.5; // Cociente objetivo para las divisiones
        ...
        function createScene() {
            ...
            // Añadir texto para mostrar el cociente objetivo con letra blanca
            const targetText = this.add.text(400, 30, `Busca pares que al dividirse den ${targetQuotient}`, {
                fontSize: '24px',
        ...
        function generatePairs() {
            ...
            // Generar pares de decimales cuyo cociente sea targetQuotient
            for (let i = 0; i < numberOfPairs; i++) {
                const num1 = (Phaser.Math.Between(10, 20) / 10).toFixed(1); // Generar decimales entre 1.0 y 2.0
                const num2 = (num1 * targetQuotient).toFixed(1); // num2 = num1 * targetQuotient
                decimalPairs.push(num1, num2);
            }

            pairs = Phaser.Utils.Array.Shuffle(decimalPairs.flat());
        }
        ...
        function checkForMatch() {
            ...
            if ((parseFloat(firstCard.value) / parseFloat(secondCard.value)).toFixed(1) === targetQuotient.toFixed(1) ||
                (parseFloat(secondCard.value) / parseFloat(firstCard.value)).toFixed(1) === targetQuotient.toFixed(1)) {
                updateFeedback(`¡Correcto! ${firstCard.value} ÷ ${secondCard.value} = ${targetQuotient}`, true);
                ...
                updateFeedback(`Incorrecto. ${firstCard.value} ÷ ${secondCard.value} no es igual a ${targetQuotient}.`, false);
                setTimeout(() => {
