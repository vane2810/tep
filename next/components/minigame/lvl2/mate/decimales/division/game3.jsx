...
const Game3 = ({ updateFeedback, updateScore, finalizeGame, resetGame }) => {
    ...
    useEffect(() => {
        ...
        function createScene() {
            ...
            // Generar y mostrar las 5 filas de preguntas de división de decimales
            for (let i = 0; i < 5; i++) {
        ...
        function generateQuestion(rowIndex) {
            const num1 = (Phaser.Math.Between(10, 50) / 10).toFixed(1); // Números entre 1.0 y 5.0
            const num2 = (Phaser.Math.Between(1, 10) / 10).toFixed(1); // Números entre 0.1 y 1.0
            const correctAnswer = (num1 / num2).toFixed(2);

            correctAnswers[rowIndex] = correctAnswer;

            // Mostrar la instrucción para la fila
            this.add.text(400, 50 + rowIndex * 100, `Fila ${rowIndex + 1}: Selecciona el resultado correcto de ${num1} ÷ ${num2}`, {
                fontSize: '18px',
                fill: '#ffffff',
                align: 'center',
                backgroundColor: '#7966ab',
        ...
        function checkAnswer(selectedAnswer, rowIndex, button) {
            ...
            if (selectedAnswer === correctAnswers[rowIndex]) {
                ...
                feedbackMessage = `¡Correcto! El resultado de la división en la fila ${rowIndex + 1} es ${selectedAnswer}.`;
