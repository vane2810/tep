// Juego 4 - División - Mate - Nivel 1 - Preguntas con tres opciones
...
const Game4 = ({ updateFeedback, updateScore, updateQuestionCount }) => {
  ...
  useEffect(() => {
    ...
    function createNewQuestion() {
      // Si se ha alcanzado el límite de 10 preguntas
      if (questionCount >= 10) {
        return;
      }

      // Generar números que se dividan sin residuo
      let divisor = Phaser.Math.Between(1, 10);
      let quotient = Phaser.Math.Between(1, 10);
      ...
      this.questionText = this.add.text(340, 200, `¿Cuánto es ${dividend} ÷ ${divisor}?`, { fontSize: '24px', fill: '#ffffff' });
      ...
      do {
        wrongAnswer1 = correctAnswer + Phaser.Math.Between(-5, 5);
      } while (wrongAnswer1 === correctAnswer || wrongAnswer1 < 0);

      do {
        wrongAnswer2 = correctAnswer + Phaser.Math.Between(-5, 5);
